...
recType *readRec(void) {
    ...
    while (1) {
        ...
        if (!eof) {
	  //	  win = &node[readTimes++].e;
	  int readLen = fread(&win->rec, sizeof(recType), 1, ifp) ;
	  
            if (readLen == 1) {
	      //		  printf("%d win-run %d lastKeyValid, %d maxRun\n", win->rec, lastKeyValid, maxRun);
	      if ((/*!lastKeyValid ||*/ compLT(win->rec.key, lastKey))
		    && (++win->run > maxRun)){
		  //  printf("%d win-run %d lastKeyValid, %d lastKey\n", win->run, lastKeyValid, lastKey);
                    maxRun = win->run;
		}
	      //                win->valid = true;
            } else if (feof(ifp)) {
	  ...
	  //            win->valid = false;
        ...
        p = win->parent;
	//	printf("extnumber %d, internalnum %d\n",p->loser->number, p->number);
        do {
            bool swap;
            swap = false;
            if (p->loser->run < win->run) {
                swap = true;
            } else if (p->loser->run == win->run &&                     compLT(p->loser->rec.key, win->rec.key)) {
...
void generateOriginalNumbers(){
  ...
  static FILE *ifp;           /* input file */
  int lessThanZero = 0;
  int orig[] = {503,87,512,61,908,170,897,275,653,426,154,509};
  char fileName[50];
  sprintf(fileName, "/bin/rm -rf in.dat");
...
static  FILE* ifp;
 ...
 //int a[10000] = {0};
  ...
  if((ifp = fopen("out.dat", "rb")) == NULL){
    printf("error: file %s, unable to open\n", "in.dat");
  }
  int j = 0;
  for(i = 0; i < 12; i++){
    recType tmp;
    int len = fread(&tmp, sizeof(recType), 1, ifp);
    if(++j % 40 == 0 ){
      printf("\n");}
    else
      printf("%d ",tmp.key);
